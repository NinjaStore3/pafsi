 ...
 *
 * Δέχεται το αίτημα της φόρμας επικοινωνίας, το επικυρώνει server-side
 * και το προωθεί με email μέσω Resend.
 *
 * Environment variables / bindings (δείτε .dev.vars.example):
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!res.ok) {
      return json(
        { ok: false, error: "Δεν ήταν δυνατή η αποστολή του αιτήματος." },
        502
      );
    }
  } catch (_) {
...
// Οτιδήποτε εκτός POST → 405 (το runtime επιστρέφει 405 αυτόματα,
// εδώ δίνουμε καθαρό JSON για τις πιο συνηθισμένες μεθόδους).
